...
describe('large-light ownership', () => {
  ...
  it.each(['initial', 'replacement', 'in-place'])('reports a non-throwing GL upload error (%s)', phase => {
    const h = harness(), runtime = new LargeLightRuntime(h.context, h.budget);
    if (phase !== 'initial') runtime.set(directional(256), []);
    const previous = runtime.texture;
    const counts = runtime.counts;
    const retainedBytes = h.budget.snapshot().retainedBytes;
    h.gl.texSubImage2D.mockImplementationOnce(() => {
      h.gl.getError.mockReturnValueOnce(1285);
    });
    expect(() => runtime.set(directional(phase === 'replacement' ? 512 : 256), []))
      .toThrow(/upload.*1285/);
    expect(runtime.texture).toBe(previous);
    expect(runtime.counts).toEqual(counts);
    expect(h.budget.snapshot().retainedBytes).toBe(retainedBytes);
    if (previous !== undefined) expect(h.gl.deleteTexture).not.toHaveBeenCalledWith(previous);
    runtime.set(directional(256), []);
    expect(runtime.counts).toEqual([256, 0]);
    runtime.dispose();
    expect(h.budget.snapshot().retainedBytes).toBe(0);
  });
