...
/** Structural KTX2 fixture. ASTC blocks are valid opaque-red LDR void-extent blocks. */
export const createKtx2Fixture = (
  vkFormat: number,
  width = 4,
  height = 4,
  levelCount = 1,
  ...
  const blockSize = vkFormat === 165 || vkFormat === 166 ? 6 : vkFormat === 171 || vkFormat === 172 ? 8 : 4;
  const blockBytes = vkFormat === 133 || vkFormat === 134 ? 8 : 16;
  const model = vkFormat === 133 || vkFormat === 134 ? 128
    : vkFormat === 137 || vkFormat === 138 ? 130
    : vkFormat === 145 || vkFormat === 146 ? 134
  ...
  for (let level = 0; level < levelCount; level += 1) {
    ...
    const byteLength = Math.ceil(levelWidth / blockSize) * Math.ceil(levelHeight / blockSize) * blockBytes;
  ...
  for (let level = 0; level < levels.length; level += 1) {
    ...
    if (model === 162) {
      for (let offset = entry.byteOffset; offset < entry.byteOffset + entry.byteLength; offset += 16) {
        bytes.set([0xfc, 0xfd, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0, 0, 0, 0, 0xff, 0xff], offset);
      }
    }
  }
  return bytes;
