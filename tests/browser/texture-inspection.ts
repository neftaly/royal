...
import { freezeInspectionSource, inspectionSampleKey } from "../../packages/renderer-webgl/src/texture/inspection-sample";
...
/** Runs in a real browser against the source tree; uses synthetic colors, never explicit imagery. */
export const runTextureInspectionBrowserTests = async (): Promise<string[]> => {
  ...
  const areaPolicy = new TextureInspectionOwner({ key: "area", allow: async image => !red(image) });
  ...
  const render = (key: string) => react.render(createElement(Canvas, {
    scene, rendererRef, style: { width: "32px", height: "32px" },
    textureInspection: { key, allow: async () => true },
  }));
  ...
  try {
    ...
    passed.push("React callback identity preserves root; policy key replaces it");
