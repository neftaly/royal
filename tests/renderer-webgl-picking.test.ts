...
describe("WebGL picking", () => {
  it("returns the intended mesh or glTF target for DOM client coordinates", () => {
  ...
  it("picks unloaded glTF fallback bounds from all transformed corners", () => {
    vi.stubGlobal("devicePixelRatio", 1);
    vi.stubGlobal("fetch", vi.fn(() => new Promise(() => undefined)));
    const root = createWebGlRoot(fakeCanvas(fakeGl()));
    ...
    expect(root.pick({ clientX: 272, clientY: 100 })?.target).toMatchObject({
      id: "rotated-gltf",
      kind: "gltf",
      node: rotatedGltf,
    });
