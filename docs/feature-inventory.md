...
| Mesh/material variants | ingestion/product | Keep. Applications select by the self-documenting `materialVariant` name, discover ready names through `useGltfAssetStatus(...).variantNames`, and can exercise the interactive demo. Declaration-order indices are intentionally not public. |
