...
| M1 | `KHR_materials_dispersion` | Adds chromatic separation for transmitted light. | Implemented for scalar `dispersion` on the private transmission screen sample using per-channel IOR spread; this is a raster approximation, not path-traced internal volume caustics. |
