...
| Functional-core boundaries | partial | Many deterministic modules and property tests exist (`lod`, frame packets, resource governor, VT demand/model, picking math, XR transitions). Availability-aware LOD selection now owns coverage, admitted-level fallback, compound node/material compatibility and stale-state cleanup in one GL-free core with retained caller-owned workspace; the WebGL owner supplies resources and submits the resulting choices. Root and other surface orchestration still contain semantic decisions worth extracting when a concrete invariant can be tested. |
...
| glTF required-extension honesty | partial | Cold preflight validates the complete declaration graph before codec work: used and required names are unique, required names are used, every payload is declared, and required placements match the executable ledger. It also rejects unknown required names, unavailable Draco, imaginary AVIF, unsupported quantization, and known names outside implemented placements. Official Khronos assets now bind punctual lights, GPU instancing, unlit, emissive strength, IOR, specular, transmission and volume lowering; clearcoat, iridescence and transformed-clearcoat fixtures prove required failure while optional clearcoat proves valid core fallback. Exact-build browser interaction smoke now covers material variants and LOD selection. WebP, Draco and every accepted texture-transform placement still need equivalent independent asset-oracle coverage before this becomes fully conformant. |
...
| Neutral texture fallbacks | partial | Canonical material factors remain authored truth, while one pure presentation planner applies the same tint-preserving 50%-sRGB fallback to missing ordinary and virtual base-color representations. Stable neutral/error contracts exist for other slots; ongoing visual oracles must ensure every material slot and VT transition avoids white/debug flashes. |
| VT demand/publication lifecycle | conforms | Pure demand/model/orchestration, transactional GPU arena, scheduling/admission/property tests and close-view stress cases provide broad evidence. Every resource's retained demand is resolved before shared-atlas admission, so protection and replacement are independent of resource insertion order. A current desktop close-view run reached distance 0.1, grew residency from 3 to 11 pages through 24 page uploads, and measured 0.4 ms renderer CPU / 1.67 ms GPU p95 while dragging. Physical Safari/Quest quality remains ongoing. |
...
| Optional WebGL capabilities | partial | HDR color-buffer, native texture compression and compatible opaque multi-draw are selected outside draws. Parallel shader compile was measured and rejected because background compilation delayed usable presentation; harness-only timer queries remain opt-in diagnostics, while anisotropy and multiview remain measurement-gated and absent. |
| Resource admission/accounting | conforms | Root governor, typed leases, capacity wake, quarantine debt, VT/texture/target/geometry tests cover physical domains. Geometry arenas are byte-chunked and claimed lazily with their first admitted surface rather than reserved for the complete scene. Bounded PNG/JPEG/WebP/AVIF dimension readers let ordinary browser decode target the already-admitted fitted size; malformed hints retain browser-authoritative fallback. Audit subsystem projections for overlap whenever new diagnostics are added. |
...
| Lazy/tree-shaken optional code | partial | XR has a separate entrypoint and several features/codecs are lazy. Bundle checks and the static architecture-fitness graph protect current boundaries; continue attributing initial/reachable gzip before adding splits. The lit/unlit compile-time alpha-mode correction consumed 68 deployed gzip bytes, so its three affected hard ceilings were raised by exactly 100 bytes. Exact public glTF metadata added 37 deployed gzip bytes and raised only the total ceiling by 50 bytes rather than hiding broad headroom. |
| Safari 17/A10 and Quest 2 physical behavior | partial | Browser/Quest harnesses and telemetry exist. Examples now embed a full revision, dirty flag, build timestamp and unique build ID in both a no-store server endpoint and every in-page report; the iPad harness rejects stale/mismatched builds and terminates boundedly after inspector failure. Current iPad WebKit and Quest DevTools transports still need fresh controlled runs; no static spec can prove sustained 60/90/120 Hz or all visual oracles. |
