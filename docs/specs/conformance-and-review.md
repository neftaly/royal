...
| Release size gates | conforms | Bundle and package budgets record exact Royal 0.0.6 references instead of ceilings that a published release already exceeded. Exact-compatible outline batching, bounded presentation, device-measured mask-normal reuse, exact scene-replacement geometry ownership, exact nested descriptor boundaries, retained multi-view outline planning, depth-prepass work, transactional creation-loss recovery, and retained screen-space segments keep one named 6,700-byte initial/total gzip allowance over that reference. Alpha-safe AVIF fitting adds a separate 156-byte lazy allowance. Royal 0.0.16 measured 136,266 initial / 143,832 lazy / 280,098 deployed / 220,790 Royal-only gzip bytes with a 56,618-byte worker; the accepted segment and shared color-presentation path measures 137,979 initial / 143,841 lazy / 281,820 deployed / 222,512 Royal-only bytes and leaves the worker unchanged. A measured lazy split was rejected because it raised complete deployed gzip and delayed cold presentation. The source-map-bearing renderer tarball measures 651,318 bytes against a narrow 652,000-byte ceiling; every budget remains a positive safe integer before packing and compiling all public entrypoints. |
