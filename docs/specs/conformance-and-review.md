...
| Open-source package shape | conforms | The repository and every publishable package ship the same canonical AGPL-3.0-only text, manifests declare that identifier and public access, and the packed-consumer check rejects a missing license or unexpected package-root file. The monorepo root and non-package applications remain `private` only as an npm publication guard; documentation describes the `0.0.5` release honestly as an open-source prerelease. |
| Release size gates | gap | The 2026-08-04 audit passes package imports plus packed-consumer structure, TypeScript and runtime-entrypoint checks, then fails the existing ratchets: initial 131,396 > 121,800 gzip bytes, incremental 72,088 > 62,500, glTF initial 132,346 > 122,750, deployed JavaScript 274,947 > 265,450, and Royal-only graph 215,639 > 206,150. The `0.0.5` renderer package tarball is 621,500 > 577,536 bytes. These are pre-existing gate gaps, not authority to raise ceilings or describe the release check as green. |
