...
| Open-source package shape | conforms | The repository and every publishable package ship the same canonical AGPL-3.0-only text, manifests declare that identifier and public access, and the packed-consumer check rejects a missing license or unexpected package-root file. The monorepo root and non-package applications remain `private` only as an npm publication guard; documentation describes the `0.0.17` release honestly as an open-source prerelease. |
| Release size gates | conforms | Bundle and package budgets record exact Royal 0.0.6 references instead of ceilings that a published release already exceeded. Exact-compatible outline batching, bounded presentation, device-measured mask-normal reuse, exact scene-replacement geometry ownership, exact nested descriptor boundaries, retained multi-view outline planning, depth-prepass work, transactional creation-loss recovery, retained screen-space segments, and exact-budget VT atlas planning keep one named 6,800-byte initial/total gzip allowance over that reference. Alpha-safe AVIF fitting and the VT planning core keep a separate 256-byte lazy allowance. Royal 0.0.16 measured 136,266 initial / 143,832 lazy / 280,098 deployed / 220,790 Royal-only gzip bytes with a 56,618-byte worker. Royal 0.0.17 measures 137,979 initial / 143,841 lazy / 281,820 deployed / 222,512 Royal-only bytes with the worker unchanged. A measured lazy split was rejected because it raised complete deployed gzip and delayed cold presentation. The published source-map-bearing 0.0.17 renderer tarball measures 651,319 bytes; the separate internal VT plan raises the current packed artifact to 652,733 bytes against a narrow 653,500-byte ceiling. Every budget remains a positive safe integer before packing and compiling all public entrypoints. |
