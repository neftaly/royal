...
| Multi-scale depth precision | deferred | Probability's golden path uses bounds-tracked conventional near/far clipping and an explicit XR depth range. Royal exposes no raw depth functions, polygon-offset parameters, render-order numbers, or conventional/reversed switch. The narrow `surfaceDepth: "contact"` presentation intent maps filled world triangles to one private constant near-depth bias without changing transforms, bounds, picking, alpha behavior, or ordering two contact layers; it is not a renderer-wide precision mode. A renderer-wide alternative would require direct, private-target and XR evidence across every depth consumer; fixed-point reversed-Z is not presumed beneficial without that measurement. |
