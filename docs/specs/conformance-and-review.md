...
| Async preparation admission | conforms | One root-owned abort-safe CPU authority bounds glTF preparation, VT work and prefiltered environments. glTF root transport is a distinct staged-byte phase because network wait does not consume that CPU authority: 16 active reads and 64 total source reservations pause at 32 MiB of completed roots until preparation begins, while one oversize source progresses. Ordinary image transport is likewise owned by its dedicated eight-request browser queue rather than consuming a glTF CPU slot; complete image-source lifecycles remain bounded at 16 active/32 total, bitmap decode at four, and completed handoff at 64 MiB. This separation moved the current 46-root Settlers final-pixel median from 19.34 to 14.52 seconds while preserving the exact transport/decode/retention ceilings. The public diagnostic projection remains separate from source and browser-stage pressure, and synchronous transition bursts coalesce into one broad tooling notification without delaying state. A pure bounded-fair selector admits newly claimed scene/environment/visible-VT work ahead of an existing detail backlog while forcing one detail start after at most four foreground starts. Embedded GLB bytes bypass the network queue. Selected-scene external geometry now crosses the same injected I/O port as bounded HTTP ranges with one complete-read fallback; physical Safari 17.14 fetched 3,606,500 of the Bistro buffer's 25,565,000 bytes in two validated responses and reached first usable presentation in 2.013 s. FIFO order remains stable within each lane, active work is not preempted, queued cancellation cannot retain dead head entries, and public diagnostics distinguish phase jobs from source reservations. The current 46-root Probability trace starts every root within 294 ms while retaining eight preparation workers. Against exact pre-change commit `a8ece04e`, the staged source lifecycle plus public phase diagnostics add 846 bytes to the ordinary Royal initial gzip graph, 4,015 bytes to the source-map/declaration-bearing renderer tarball, and no worker payload; the exact byte cost is accepted for the causal request-span and rendered-draw improvement rather than hidden in unmeasured headroom. Cross-domain transient-byte weighting is deliberately rejected: browser decode/worker scratch is not accurately knowable up front, while staged roots, texture handoff, codec, VT, environment, persistent GPU and upload domains own enforceable bounds. |
...
adds no frame, GL, picking, texture-decode, or retained geometry path. Only the
