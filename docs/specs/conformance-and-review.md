...
| Per-frame upload admission | partial | Separate root-local byte governors defer ordinary-texture and canonical geometry/instance transfers consistently across canvas and XR frames. Geometry transactions admit an exact progressive prefix and allow one oversize first transaction to prevent starvation; texture-only commits cannot consume geometry allowance. VT page transfer and render-target allocation remain separately governed or still need an explicit traffic-domain justification. |
