...
refetching. Extension dispatch remains gated on one logical fallback lifecycle.
...
The single-source SVG/VT handoff adds 44 lazy and 82 deployed gzip bytes while
leaving initial Royal unchanged; only those two ceilings rise by 100 bytes. Its
source and declarations add 165 bytes beyond the prior packed renderer ceiling,
which rises by one KiB.
