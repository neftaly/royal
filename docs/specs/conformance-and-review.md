...
| glTF required-extension honesty | partial | Cold preflight validates the complete declaration graph before codec work: used and required names are unique, required names are used, every payload is declared, and required placements match the executable ledger. It also rejects unknown required names, unavailable Draco, imaginary AVIF, unsupported quantization, and known names outside implemented placements. Official clearcoat, iridescence, and transformed-clearcoat fixtures prove required failure while optional clearcoat proves valid core fallback. Remaining accepted profiles still need official per-extension conformance oracles before this becomes fully conformant. |
...
| Hot-path GC discipline | partial | Many retained typed workspaces and high-water capacities exist. Root/frame and focused asset observers share one mutation-safe retained listener owner; publication no longer allocates listener snapshot arrays. Canonical indices are validated before the GL shell and use one allocation-free rebasing pass during upload. Targeted allocations remain in LOD identity, VT admission/demand, binding readiness and public snapshot paths. Trace first and preserve clarity. |
