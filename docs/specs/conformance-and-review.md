...
| Release size gates | conforms | Bundle and package budgets record exact Royal 0.0.6 references instead of ceilings that the published release already exceeded. Exact-compatible outline batching receives a named 2,500-byte gzip allowance over that reference; the checked initial graph, deployed graph, Royal-only graph, lazy chunks and worker remain within their ratchets. Packed-consumer budgets likewise retain the published 622,515-byte renderer tarball as their reference and validate every budget as a positive safe integer before packing and compiling all public entrypoints. |
