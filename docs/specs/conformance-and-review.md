...
| Safari 17/A10 and Quest 2 physical behavior | partial | Browser/Quest harnesses and telemetry exist. Examples embed a full revision, dirty flag, build timestamp and unique build ID in a no-store server endpoint and reports. Smoke, route-benchmark and glTF-load harnesses require the served identity to match the current emitted build; managed runs also require their own preview readiness, so an occupied port cannot silently redirect evidence to an older process. The iPad harness rejects stale/mismatched builds and terminates boundedly after inspector failure. Current exact clean Safari build `6e6047ef` has a complete texture-integrity pass. Sponza settles 69/69 images with zero fallback, failure, GPU denial or browser diagnostics, then completes 60/60 full-DPR motion samples at 15.95 ms average and 19 ms p95. Selector-addressed Bistro Exterior/Interior/Interior Wine settle every 202/111/110 image without fallback, failure, denial or browser diagnostic and complete 30/30 moving frames at 34/28/24 ms p95. Exact clean Quest build `278cbdc7` closes the missing device row: the Adreno 650 enters one 90 Hz immersive session with two 2880x1584 views, completes 180/180 samples at 11.27 ms average / 14.34 ms p95 and 1.00 ms Royal-callback p95, settles the Tiger SVG without fallback/failure, retains a coherent stereo framebuffer, and explicitly exits. Its close-ground VT run reaches distance 0.1, grows from 5 to 11 resident pages without pending/failure, and completes active close-view frames at 11.2 ms p95. Both bounded runs retain Quest 2/Android 14 identity, memory, filtered logcat and before/after thermal telemetry: status remains `none`, battery temperature stays 34.5–34.7 °C, GPU sensor peaks at 57.63 °C, no error/fatal/assert event is retained, and every probe succeeds. The Quest run meets the accepted 60 Hz floor, while its 14.34 ms p95 does not establish sustained 90 or 120 Hz. Sponza's iPad p95 remains above 16.67 ms, while Bistro and A Beautiful Game remain below the desired device floor; those are measured performance constraints rather than missing evidence. |
