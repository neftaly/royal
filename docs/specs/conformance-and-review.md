...
| Cross-root canonical geometry preparation | proposed | Probability proves 39 roots in four exact geometry groups, but Royal's current whole-asset workers still derive geometry identity from root content plus mesh/primitive indices and repeat accessor conversion/canonical packing across roots. A worker-local cache would retain detached transfer storage or copy every output; a main-thread cache would remove only GPU uploads; sending cached arrays through another whole-asset worker would add two copies. The accepted target is a two-stage protocol: validated geometry work identities, a claim-aware root geometry owner publishing arrays once, and independent scene/material skeleton composition. URL-only keys, weak hashes, and GPU-only interning are not counted as conformance. |
