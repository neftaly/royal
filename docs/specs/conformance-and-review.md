...
| Canonical texture upload plan | conforms | Ordinary images lower to one decoded-source union with exact compressed/uncompressed budgeting. Ordinary and VT offline ETC2 share one validated KTX2 block parser, block-storage vocabulary, color-space rule and WebGL-format selector. The root enables `WEBGL_compressed_texture_etc` once per context generation; direct and authored-VT compressed sources reject before invalid upload when absent. Direct ETC2 storage is a Royal source capability, not a private glTF extension. One pure glTF planner selects draft AVIF, WebP, or core and fetches only one representation; optional SVG recovery uses the same ordered raster plan. Ordinary AVIF budget fitting deliberately resamples native decoded pixels through a canvas because Firefox corrupts alpha in both encoded-source and decoded-bitmap `createImageBitmap` resize operations; native AVIF and direct PNG/WebP fitting retain their existing paths. An optional higher-priority source may omit core only when a present lower-priority extension is itself required; an all-optional chain without core fails authoring validation. VT pages deliberately retain a narrower independently scheduled transport record: a page is not a complete texture/mip chain, and forcing page identity/residency into the ordinary union would couple lifecycles without simplifying binding or drawing. glTF Basis remains an honest non-claim rather than a hidden transcoder branch. |
