...
| Open-source package shape | conforms | The repository and every publishable package ship the same canonical AGPL-3.0-only text, manifests declare that identifier and public access, and the packed-consumer check rejects a missing license or unexpected package-root file. The monorepo root and non-package applications remain `private` only as an npm publication guard; documentation describes the `0.0.15` release honestly as an open-source prerelease. |
| Release size gates | conforms | Bundle and package budgets record exact Royal 0.0.6 references instead of ceilings that the published release already exceeded. Exact-compatible outline batching, bounded presentation, device-measured mask-normal reuse, exact scene-replacement geometry ownership, exact nested descriptor boundaries, retained multi-view outline planning, depth-prepass work, and transactional creation-loss recovery receive one named 5,100-byte gzip allowance over that reference. The recovery path measures 136,261 initial / 279,916 deployed / 220,608 Royal-only gzip bytes; lazy chunks and the worker do not grow. Packed-consumer budgets likewise retain the published 622,515-byte renderer tarball as their reference. Transactional construction and public diagnostics move the renderer tarball to 642,609 bytes, so its narrow ceiling rises from 641,000 to 643,000; every budget remains a positive safe integer before packing and compiling all public entrypoints. |
