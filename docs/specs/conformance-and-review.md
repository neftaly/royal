...
| Open-source package shape | conforms | The repository and every publishable package ship the same canonical AGPL-3.0-only text, manifests declare that identifier and public access, and the packed-consumer check rejects a missing license or unexpected package-root file. The monorepo root and non-package applications remain `private` only as an npm publication guard; documentation describes the `0.0.14` release honestly as an open-source prerelease. |
| Release size gates | conforms | Bundle and package budgets record exact Royal 0.0.6 references instead of ceilings that the published release already exceeded. Exact-compatible outline batching, bounded presentation, device-measured mask-normal reuse, exact scene-replacement geometry ownership, exact nested descriptor boundaries, and retained multi-view outline planning receive one named 4,100-byte gzip allowance over that reference; the checked initial graph, deployed graph, Royal-only graph, lazy chunks and worker remain within their ratchets. Packed-consumer budgets likewise retain the published 622,515-byte renderer tarball as their reference. The direct-multisample outside-coverage classifier moves the preceding 639,366-byte measurement to 640,332 bytes, so only the renderer ceiling rises from 640,000 to 641,000; every budget remains a positive safe integer before packing and compiling all public entrypoints. |
