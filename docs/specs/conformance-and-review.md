> Historical SVG measurements below describe the removed implementation; SVG texture support is no longer available.
...
Attack: keep two renderers, preserve abandoned owners behind new names, copy
whole subsystems, let old screenshots define correctness, or keep old modules
as an automatic fallback.

Resolution: one production path owns each behavior. Version-control history is
evidence, not runtime authority. A fallback must preserve the same semantics
for a documented capability or failure case; it is not permission to retain an
abandoned subsystem.
...
| Bounded emissive volumes | partial | The public standalone node validates copied density/noise/extinction inputs, rejects zero scale eagerly, and lowers boxes plus closed outward convex triangle hulls before GL allocation. Its lazy WebGL owner retains exact proxy values rather than collision-prone compact keys, handles external perspective views, and retries budget denial only after released capacity. Direct scenes keep their established surface shaders, destination, alpha behavior, and default-framebuffer multisampling: opaque color renders normally, a private RGBA8/depth target reproduces occlusion only, and presentation-mapped volume emission is inserted before transparent work. A pure capability plan omits an unsupported volume when an established RGBA16F composite lacks float blending instead of downgrading all radiance. Focused descriptor, hull, ownership, collision, budget, packed-consumer, capability-matrix, byte-identical zero-contribution screenshot, visible-route, and context-restoration checks cover the shipped path. The implementation is deliberately a twelve-step full-resolution emissive approximation: direct volume emission is tone-mapped before source-over blending, transparent surfaces are always later, transmission snapshots exclude volume emission, overlapping media use ordered alpha, and no physical Safari/Quest performance evidence exists yet. |
| Euler transform convention | conforms | Every public `EulerRads` tuple composes exactly as Three.js's default `Euler(x, y, z, "XYZ")`. A direct Three.js oracle covers identity, single-axis, multi-axis, negative, gimbal-boundary and deterministic generated rotations; camera views are the inverse of the same object transform, orbit cameras retain their target through XYZ decomposition, and packed Euler instance streams share the ordinary matrix core. glTF quaternions and explicit matrices remain independent. No order option or legacy ZYX path exists. |
| Screen-space complementary coverage | partial | One validated descriptor feeds unlit fill, fixed-width overlay segments, and edge presentation through one root-shared lazy R16UI pattern; canvas CSS scaling, XR view-local presentation pixels, exact complementary buckets, overlay sharing, fragment discard/alpha-only edge masking, and presentation-independent picking have focused tests. Physical iPad and stereo comfort evidence for the Probability use case remains outstanding. |
| Always-visible overlays | partial | `SceneOverlay` and `setOverlay` provide one non-picking depth-independent lane. Direct solid meshes, retained instanced `screenSpaceSegment` guides, and glTF edge outlines publish independently in an explicit kind-lane order; segment endpoint replacement touches only one root-budgeted float32 overlay buffer, while unchanged canvas/XR frames upload no expanded geometry. Real Chromium, Firefox, and WebKit captures retain the same 4-CSS-pixel guide width at different perspective depths with no frame failure. The exact deployed 0.0.17 build rendered both segment guides on physical Quest 2 and recorded 20 real array-instanced submissions across 20 animated frames with no runtime, console, or VAO-ownership errors. Outlines borrow the rendered source occurrence's prepared geometry, instance cohort, active LOD and GPU allocation, including displaced previews selected through `sourceTransform`. Coincident occurrences with the same source identity and exact transform are presentation-equivalent and may share any ready resident geometry. Automatic instance batches retain same-index source asset, primitive, mounted-occurrence and float32 transform provenance, so one ordinary outline mask can borrow a non-representative same-root or cross-root member without another geometry upload, an instanced mask draw, or world re-lowering. Exact-compatible outline occurrences additionally batch their complete primitive sequence occurrence-major, with optional allocations admitted behind required targets and ordered-draw fallback on denial or setup failure. Combined index preparation now binds the non-drawing default VAO instead of mutating the last world VAO. A semantic VAO model, deterministic state-sequence property test, and mutation-tested real-WebGL browser probe independently check retained element bindings, sticky buffer targets, deletion, restoration, and the actual index buffer consumed by every indexed draw. The forced Tiger route must observe the exact default-VAO preparation as well as a batched instanced draw. The resource-dependent plan survives frames and views, while a retained visibility union omits occurrences outside every submitted view from the one packed transform upload shared by stereo draws. Authored instance cohorts keep whole-cohort behavior; missing sources still fail. A root-budgeted retained presentation avoids complete world redraws for warm overlay-only changes and falls back to a correct world render on denial. Focused WebGL and Probability browser tests cover ownership and update isolation; a physical Safari 17.14/Apple GPU 72-object displaced drag confirms correct low-tens submissions with no GL errors. A matched physical Quest 2/Adreno 650 immersive A/B reduces 72 displaced outlines to two mask submissions per stereo frame with no page errors. The original complete overlay raised median frame intervals from 8.36 ms to 21.33--22.23 ms. Exact mask/normal reuse, paired binary resolve, depth discard, and conservative sampled-pass scissors now retain eight submissions per frame and measure 8.38--8.39 ms median / 13.49--14.23 ms p95 across two warm-up-inclusive 600-frame runs. After the first 120 intervals, their steady windows measure 8.33--8.35 ms median / 9.28--9.44 ms p95. This meets the stable 60 Hz floor without a resolution or browser policy. The WebXR Tiger example now defaults to one solid presentation instead of its shimmer-prone three-way one-pixel partition, halves submissions to 22 per frame, retains one stable 21-page VT snapshot in a resident repeat, and measures 8.37 ms median / 9.69 ms p95 after entry. Sustained 120 Hz remains open. |
| Multi-scale depth precision | deferred | Probability's golden path uses bounds-tracked conventional near/far clipping, an explicit XR depth range, and physical separation for visually distinct world surfaces. Royal exposes no raw depth functions, polygon-offset parameters, render-order numbers, inferred contact mode, or conventional/reversed switch. Equal stored depths already pass in submission order; a generic near-equality rule would require a tolerance or bias that can override physically distinguishable geometry and couple correctness to batching and backend precision. Royal therefore explicitly rejects automatic coplanar tie-breaking. A renderer-wide alternative would require direct, private-target and XR evidence across every depth consumer; fixed-point reversed-Z is not presumed beneficial without that measurement. |
| Non-visual glTF ownership and geometry | conforms | Declarative `gltfAssetClaims` and imperative `setGltfAssetClaims` retain exact identities through the ordinary preparation, cancellation, status, borrowed-geometry, and selected material-image lifecycle without scene, picking, light, GPU, or frame work. Claims made before the first scene still prepare those render-ready CPU resources. Equivalent descriptors and visual/non-visual handoff preserve one preparation. Status observation remains non-owning. Prepared bounds are explicitly a conservative framing/coarse-layout/broad-phase AABB rather than contact or support geometry; consumers derive and own specialized structures through the cold borrowed-geometry visitor. Focused owner/root tests cover cancellation, duplicate claims, handoff before and after settlement, render-ready image preparation, and absence of GL/frame side effects; the packed consumer compiles the React API. |
| React ordinary-tree ownership | conforms | `Canvas` owns one root and controls/hooks remain normal React children; cleanup and public API tests exist. React and imperative hosts share the self-documenting `pixelRatio` policy, invalid explicit ratios fail at the React boundary, and capability clamping remains distinct as `renderScale`. Every focused asset, renderer, and XR lifecycle narrows through the same `status` discriminator without a React adapter allocation. Drawable glTF status carries the resolved document scene index, lightweight `{ index, name? }` inventory, variant names, and optional uninterpreted root `extras`, so React controls do not parse a second glTF or prepare unselected scene content. Extras take one cold ownership copy, remain reference-stable through texture progress, and cannot mutate the renderer's prepared artifact. Public root diagnostics use the same `imageTexture` vocabulary as authoring rather than the internal “ordinary” qualifier, while standalone scheduler/upload-policy types remain private behind `RendererResourceSnapshot`. The packed-consumer fixture compiles the primary React composition, focused parent/child observation, document scenes, glTF root extras, variants, instances, custom picking geometry, XR and the imperative escape hatch while rejecting scene-only and internal browser-port or resource-policy types from runtime entrypoints; exact runtime export tests also keep the main, scene, XR, and lower WebGL barrels ownership-oriented. |
| Open-source package shape | conforms | The repository and every publishable package ship the same canonical AGPL-3.0-only text, manifests declare that identifier and public access, and the packed-consumer check rejects a missing license or unexpected package-root file. The monorepo root and non-package applications remain `private` only as an npm publication guard; documentation describes the `0.0.19` release honestly as an open-source prerelease. |
| Release size gates | conforms | Bundle and package budgets record exact Royal 0.0.6 references instead of ceilings that a published release already exceeded. Named allowances cover accepted optional capabilities rather than silently moving ceilings. Royal 0.0.17 measures 137,979 initial / 143,841 lazy / 281,820 deployed / 222,512 Royal-only gzip bytes. Against the clean pre-feature 0.0.18 branch, lazy bounded volumes added 1,286 bytes to the initial graph and 4,719 bytes to the complete deployed/Royal-only graph; 3,216 bytes were isolated in the new volume chunk. The 0.0.19 color-parity correction adds another 245 initial / 392 lazy / 637 deployed and Royal-only gzip bytes. The resulting build measures 139,506 initial / 147,759 lazy / 287,265 deployed / 227,957 Royal-only bytes with the 56,618-byte worker unchanged. Its source-map-bearing renderer tarball measures 672,713 bytes against a 674,000-byte ceiling. Every budget remains a positive safe integer before packing and compiling all public entrypoints. |
...
| Context generation/lifecycle | conforms | Loss/restoration listeners precede the context request; null and lost creation failures have distinct public diagnostics, and a reverse-order construction transaction releases every completed owner before removing its listeners. React waits for a real restoration event and retries once on the same canvas generation rather than polling or replacing it. Focused tests cover pre-existing loss, loss during capability reads, late-construction rollback, cleanup, ordinary loss/restore/disposal, and the React recovery owner; an opt-in real-WebGL browser smoke forces loss inside the initial `getContext`, observes one restoration and a second attempt on the identical sole canvas, then renders the scene. Royal deliberately owns no `pagehide` context-loss policy. |
| Retained scene/packet path | partial | Canonical retained tables/workspaces exist. Stable fixed state, program, bindings and VAO now compile into one draw packet shared by ordinary and multi-draw submission; framebuffer/viewport remain a per-view frame packet. Asset-local LOD groups lower to dense prepared IDs and then dense scene-local numeric occurrence IDs shared by visual and picking selection; array indexing builds their retained group table without compound mount strings. Per-frame LOD state now keeps that dense representation through typed high-water storage shared by color, depth and picking instead of reintroducing `Map`/`Set` hashing and iterator cleanup. Geometry arena plans compare their retained structure directly instead of serializing a parallel identity graph, and instanced VAOs use a collision-free length-framed geometry/cohort key without temporary tuple serialization. Independently prepared visual glTF roots publish focused status and texture claims immediately but coalesce structural scene/GPU publication at the root's next canvas or XR frame; imperative picking flushes the same pending transaction. A 24-root oracle observes one GPU scene transaction, while external/embedded/MASK texture and XR oracles prove texture work is neither delayed nor split into another lifecycle. Mesh/glTF render-object refs now enter one indexed retained-transform core shared by visual matrices, bounds/LOD, authored lights and exact picking; the WebGL shell updates only affected packet handedness/light uniforms, and exact-size multi-draw run storage is reused across pointer-rate changes. Focused object/callback/shared-ref, progressive glTF, declarative sync, negative-scale and no-`setScene` oracles cover the path. Some other commit/topology construction still allocates readable cold arrays/maps; profile it before adding retained machinery. |
| Cross-material automatic instancing | deferred | Exact-material geometry cohorts already converge automatically. A deliberately incorrect Probability deck upper bound that ignored distinct front textures reduced submissions by 82--86% (roughly 62 per frame to 9--11) without improving the 73-to-292-piece SwiftShader scaling slope. On physical Safari/A10, the same wrong-texture collapse reduced a 52-card SVG workload only from roughly 49 ms to 44 ms median. A correct per-instance material/VT selector would add shader, indirection and publication lifetime; it remains absent until a representative physical A/B demonstrates an end-to-end gain that justifies those costs. |
...
| Canonical texture upload plan | conforms | Ordinary images and authored VT share one native KTX2 parser, block-format table, color-space contract, and exact GPU/upload accounting for ETC2 RGBA, ASTC LDR 6x6/8x8, and BC1 RGBA/BC3/BC7. Device-unsupported native sources settle to the neutral slot fallback, release preparation reservations, and never issue invalid compressed uploads. Capability checks are cold and caches reset after context loss. Format/extent/color-space atlas keys separate incompatible pages. Pickable MASK sources still require raster/ETC2 alpha or explicit picking proxies; ASTC/BC CPU alpha decoding is not implemented. No runtime encoders or Basis transcoders are shipped, and this is not a private glTF extension. Regression tests cover both transfer functions, partial blocks, mip fitting, context loss, unsupported devices, page admission, and exact byte accounting; `research/native-texture-formats` records software-browser and CPU benchmarks, without claiming native GPU performance. |
...
| `MSFT_lod` semantics | conforms | Node/material chains, variants, thresholds, terminal cull, hysteresis, stereo selection and readiness fallback are tested. Material base-color preparation is lowest-first across external early discovery and canonical embedded claims; the retained selector admits a preferred material level only after its base image is GPU-resident, so upload pacing or failure preserves the nearest resident preview. Repeated mounts of one prepared asset lower to independent dense numeric occurrence IDs and world bounds rather than coupling selection through an asset-global string. Lowest-first geometry publication is only a deferred load optimization. |
...
| Transparent picking semantics | conforms | Blended materials deliberately use their triangle surface regardless of per-texel/factor alpha; only `MASK` materials attach the canonical alpha predicate. The canonical-lowering regression proves that an ordinary transparent surface retains neither mask texture data nor a mask sampler. Per-texel blended-alpha picking remains an explicit non-feature. |
| Optional WebGL capabilities | partial | HDR color-buffer, native texture compression, and exact-compatible multi-draw are selected outside draws. Opaque runs use retained plans; sufficiently large sets of coverage-independent standard draws add one position-only, color-suppressed depth pass while the camera remains inside their aggregate volume. Outside cameras require both a direct multisampled default target and at least 2× retained-bound coverage along the dominant outside axis; sparse views, single-sample composites, alpha mask/blend, transmission, lines, unlit work, and the no-extension fallback remain single-pass. A 5% exit margin prevents boundary churn. Physical Safari measured Sponza at 39 ms p95 without the pass and 23 ms with the camera-volume plan. A corrected wall-clock-path Settlers A/B measured the retained outside classifier at 33 / 39 ms median/p95 against 37 / 44 ms control, while the cleaned candidate repeated at 30 / 38 ms. Globally forcing the pass had separately regressed Bistro Exterior from 33 / 36 ms to 34 / 38 ms and 147 to 192 submissions; the constrained classifier preserves its 147-draw composite path. Depth and color vertex programs declare the same clip position invariant: an earlier physical Safari counterexample showed severe self-occlusion when cross-program depth drift was left implementation-defined, while the restored pass produced the same clean Sponza view as the no-prepass control. Covered color draws require exact depth equality while all other work retains less-or-equal comparison. Alternating physical Settlers runs measured equality at 27 / 34 and 26 / 34 ms median/p95 versus less-or-equal controls at 28 / 55 and 31 / 38 ms; the first control tail was noisy, but the median separation survived order reversal. Depth-writing transmission retains exact-state runs and sorts only inside them; alpha-blended transmission remains globally back-to-front. On physical Safari this reduced moving Bistro submissions from 181 to 91 per frame, program switches from 32 to 15 and VAO binds from 124 to 35 while normalized capture RMSE remained 0.000303. Frame p95 remained 23 ms, isolating the next constraint to fragment/texture work rather than JS submission. Successful shader setup performs one link-status synchronization instead of polling every stage; the shared compiler/linker shell owns diagnostics and shader cleanup for presentation, surface, and depth programs. When `KHR_parallel_shader_compile` is available, ordinary surfaces start deduplicated untextured and fully authored variants when the canonical scene becomes known, then the unchanged draw path synchronizes and publishes only on first use. Browsers without the extension keep the previous synchronous path. On the measured Intel renderer this reduced median synchronous link waiting from 283 to 119 ms and final pixels from 2.214 to 2.173 seconds; physical Safari and Quest timing remain unclaimed. Deferring complete surface publication until background links finished was separately rejected because it delayed usable presentation. Harness-only timer queries remain opt-in diagnostics, while anisotropy and multiview remain measurement-gated and absent. |
...
| Async preparation admission | conforms | One root-owned abort-safe CPU authority bounds glTF preparation, VT work and prefiltered environments. glTF root transport is a distinct staged-byte phase because network wait does not consume that CPU authority: 16 active reads and 64 total source reservations pause at 32 MiB of completed roots until preparation begins, while one oversize source progresses. Ordinary encoded-image transport runs ahead under a separately observable 16-read, 128-source, 32 MiB staged-blob authority. Complete image-source lifecycles remain bounded at 32 active/64 total, browser decode admission at 32, and completed handoff at 64 MiB; the browser, not Royal, chooses its codec thread count. Render-ready non-visual claims discover those sources without creating scene or GPU work. The controlled Probability experiment starts all 45 AVIF requests by about 2.2 seconds; under concurrent workstation load, the complete branch reached its last changed frame in 9.70 seconds versus 18.04 seconds with the prior 4 MiB/four-decode/16-preparation policy. The earlier isolated 16-to-32 preparation experiment improved median completion by 0.95 seconds. These are scheduling/contention results rather than physical-device frame claims. FIFO order remains stable within each lane, active work is not preempted, queued cancellation cannot retain dead head entries, and public diagnostics distinguish phase jobs, root reads, encoded image reads, decoded handoff, and upload pressure. Embedded GLB bytes bypass the network queue. Cross-domain transient-byte weighting remains rejected: browser decode/worker scratch is not accurately knowable up front, while staged roots, texture handoff, codec, VT, environment, persistent GPU and upload domains own enforceable bounds. |
| glTF preparation worker lifetime | conforms | Worker-worthy glTF preparation borrows a root-owned amortized set bounded by the existing job ceiling rather than creating a second queue or one worker per asset. Success and ordinary asset failure permit reuse for a one-second idle grace; cancellation, worker/channel failure, idle expiry and root disposal terminate the affected worker. Source, resource and prepared storage keep their transfer ownership without defensive copies. The current 46-root Probability Settlers trace records eight worker-host starts and eight worker-module compilations versus 32 actual starts in the pre-change raw trace. A five-second grace retained eight starts and changed current-build LCP only from 1.107 to 1.110 seconds, so the longer memory lifetime was rejected. The reusable shell adds roughly 0.1 kB gzip to the initial path and 0.5 kB to lazy chunks; affected initial/deployed ceilings rise by 50–650 bytes and the source-map-bearing packed-renderer ceiling by 2 KiB, while the worker asset is unchanged. |
| Cross-root canonical geometry preparation | conforms | Eligible small external JSON roots derive complete geometry task identities before worker admission. A root-owned claim-aware owner chooses one producer; concurrent joiners immediately prepare root-specific material/scene skeletons while omitting borrowed geometry buffer demand, codec/accessor conversion, canonical allocation and transfer. Only final placeholder resolution waits for the producer. A joiner temporarily retains one bounded root retry copy, so failure or cancellation releases claims and lets surviving roots retry ordinary preparation without serializing their common path. Identity includes immutable resource URI/version, declared length, topology, accessors, views, compression, quantization and required UV representation. Focused fixtures prove one resource read, one preparation task and one GPU upload across roots with distinct materials/transforms, plus skeleton progress before a blocked producer resource. GLB and otherwise ineligible roots retain byte-exact post-transfer interning as a correctness-preserving fallback. The task owner shares the early-root lazy boundary; on-claim preload leaves only a small promise shell initial. Settled initial/lazy/worker/deployed gzip is 121,110/143,250/56,607/264,360 bytes. Probability's complete-bundle follow-up reported a 17.15-second median final changed frame versus 20.25 seconds on clean `609052de`; Royal's direct no-gate reproduction and transport-admission correction then reported a 14.52-second median over three cold runs. |
| Per-frame upload admission | conforms | Separate root-local byte governors defer ordinary-texture, canonical geometry/instance, and VT page/page-table transfers consistently across canvas and XR frames. Each progressive domain allows one oversize first transaction to prevent starvation; texture-only commits cannot consume geometry allowance. Geometry has no redundant surface-count throttle: one exact byte-governed transaction publishes every following surface whose resident geometry/instance references cost zero upload bytes. A 719-surface focused fixture therefore completes with one 54-byte geometry upload, while distinct uploads retain their progressive prefix. Intermediate geometry and texture batches commit without redrawing an unchanged scene, while first-usable, terminal, camera, scene and application presentations remain urgent. On the current Bistro web tier this cut startup draws from 3,185 to 1,110 and load-frame GPU p95 from 13.4 ms to 7.36 ms without changing the 110/110 terminal texture outcome. A later clean `d6dee369` run against the checksum-current fixture reached first usable in 494 ms, settled 110/110 images in 2.21 s and reached terminal readiness in 2.60 s on desktop; it does not replace the earlier controlled before/after comparison. The atomic environment profile has a hard 2,097,144-byte format ceiling. Render targets and GPU-to-GPU scene-color copies are capacity/bandwidth domains rather than client-source transfer. |
| Hot-path GC discipline | partial | Many retained typed workspaces and high-water capacities exist. Root/frame and focused asset observers share one mutation-safe retained listener owner; publication no longer allocates listener snapshot arrays. Canonical indices are validated before the GL shell and use one allocation-free rebasing pass during upload. Geometry arena preparation constructs each chunk entry once and avoids the former slice/map/JSON identity graph; semantically stable React XR option lists are retained without serializing on every render. The glTF sampler reader normalizes its closed numeric enum without constructing a lookup `Map` per texture. Retained transmission visibility is dense over actual candidates and eyes rather than sparse over every scene surface; its stereo classification, roughness demand and target extent reuse one per-root high-water workspace. Scene-local numeric LOD identities remove retained compound strings, and the frame selector keeps levels plus current/previous activity in typed high-water arrays rather than `Map`/`Set` state. Progressive settlement scans retained texture identities without a success-path callback or empty-array allocation in the root. A texture-ready batch also deduplicates overlapping material-map identities into one dense high-water surface pass, so one material is not rebound and reclassified once per map that settled in the same frame. Shared-atlas admission returns a retained-state slot integer rather than allocating a plan for each uploaded VT page. VT demand retains only a deduplication `Set` plus direct typed-array/count/overflow state; it does not store unused per-page indices or a wrapper record for one mutable bit. Fixed-unit texture composition writes into the draw packet's one retained binding array, so no-op texture publication neither allocates a discarded array nor retains a duplicate pointer. Light, material and scene-uniform lowering reuse owner-retained vec4 storage and create no per-material arrays or option objects. Outline borrowing scans retained geometry without per-call candidate collections, resource-stable mask plans survive frames and views, and the owner reuses its transform, visibility, map, set, packet, and clear-intent workspaces; stereo views share one packed upload rather than rebuilding view-specific batches. The publication workspace costs about 0.2 kB gzip in every Royal app and is accepted because its work reduction scales with both maps per material and surfaces per shared image; the four affected bundle ceilings rise by 0.2 kB with no lazy or worker increase. A 90-frame desktop Sponza trace measured ~0.2 ms renderer callback p95 and no GC pause signal. Exact commit `df631f9c` records 298 ms across 240 physical-Safari callbacks while 120 motion samples average 16.5 ms and reach 19 ms p95; the remaining tail is not evidence for a JS/GC rewrite. At exact commit `3514a897`, an uninstrumented 180-frame moving Virtual City profile measured 0.6 ms renderer-callback p95 and 6.96 ms GPU p95. Forced-GC controls retained about 111 KiB for that 198-submission scene versus 69 KiB for a one-draw cube; the 42 KiB difference was bounded across the complete run, while full GL-call interception raised callback p95 to 2.4 ms. This rejects both a frame-proportional leak diagnosis and allocation work inferred from instrumented overhead. Targeted allocations remain in VT demand and public snapshot paths. |
...
| Lazy/tree-shaken optional code | partial | XR has a separate entrypoint; VT/SVG, environment transport, browser preparation and codecs are lazy. One pure VT activation decision distinguishes authored demand from opt-in automatic base-color demand and is reused by setup and stale-import guards; the root alone owns the dynamic import. Bundle checks and the static architecture-fitness graph protect current boundaries; continue attributing initial/reachable gzip before adding splits. The current audit rejected another split: glTF is baseline product behavior, picking must remain synchronous at the root API, and the remaining optional systems already sit behind dynamic imports or separate entrypoints. The measured pure glTF authoring fixture is now a separate budget: it adds about 1.1 kB gzip over the Royal React initial path without pulling codecs or workers into that path. Explicit authored-normal shader selection consumes about 170 deployed gzip bytes and raises its affected rounded ceilings by 100–200 bytes. The lit/unlit alpha correction consumed 68 deployed gzip bytes and raised its three affected ceilings by 100 bytes. Exact public glTF metadata added 37 bytes and raised only the total ceiling by 50; reachable transmission mip planning added 68 bytes and raised only that ceiling by a further 100. Ordered transparent batching plus thin/volume shader specialization consumed about 90 deployed gzip bytes across two measured slices and raised the total ceiling by 100. Retained transmission grouping added under 0.7 kB to the packed renderer artifact while halving physical Bistro submission calls. Resource-only geometry publication adds 103 deployed gzip bytes for the measured 65% Bistro startup-draw reduction; its affected ceilings rose by 10–140 rounded bytes. Bounded-fair foreground/detail preparation costs about 0.2–0.3 kB gzip in the always-reachable root and raises its four affected ceilings by 200 bytes; it adds no worker or lazy-chunk payload. Retained scene-uniform and composite-frame functional cores add about 0.3 kB gzip to the initial/deployed path; their four affected ceilings rose by 350 bytes, with no lazy or worker increase. Exact ordinary-texture byte/compression diagnostics add 27–44 gzip bytes across the three initial fixtures. Heavy-scene texture fitting and the camera-volume depth plan add about 0.5 kB gzip together to prevent partial Bistro residency and reject harmful outside-view prepasses; their four affected rounded ceilings rose by 600–700 bytes in total, while lazy and worker ceilings remain unchanged. The direct-multisample outside-coverage extension adds 34–86 gzip bytes across affected initial/deployed paths; its five narrow ceilings rise by 50–150 bytes with no lazy or worker change. Invariant-prepass depth equality adds 10–75 gzip bytes across the same synchronous graphs; its five narrow ceilings rise by 50–100 bytes and lazy/worker ceilings do not change. Lightweight document-scene metadata remains in lazy glTF preparation: it raised the rounded lazy/total/worker ceilings by 200/200/100 gzip bytes and the packed renderer ceiling by 1 KiB without changing the ordinary React initial gate. Unified explicit-root picking and invalidation add under 0.2 kB gzip to affected initial/deployed fixtures without changing lazy or worker payload. Physical-pixel alpha-mask footprints and mip-filter matching add about 0.9 kB gzip to the synchronous path and 0.3 kB to lazy decoding after the cold mip generator was kept out of the initial graph; the affected initial, lazy and total ceilings rose by 0.9/0.3/1.3 kB, and the source-map-bearing packed renderer ceiling rose by 7 KiB. The explicit root-owned VT activation lifecycle adds under 0.1 kB gzip to the measured initial path and no lazy payload; the affected initial ceilings rose by 100 bytes and the total ceiling by 50. Exact light-count specialization, API-policy cleanup, phase-separated texture preparation, image-texture diagnostic vocabulary, transmission tinting, and selected-material lighting demand leave the measured initial/deployed fixtures at 114,476/229,036 gzip bytes. The final lighting-demand rule costs under 0.1 kB gzip while removing inert IBL/light ownership; affected ceilings rise by only 100 bytes to the next rounded boundary. Dense typed LOD generations plus allocation-free root settlement scanning add under 0.1 kB gzip to the synchronous/deployed path and nothing to lazy or worker payloads; only the four affected ceilings rise by 50 bytes. The linear VT page-table resolver adds 24 deployed gzip bytes only; the total ceiling rises by 50 bytes with no initial, lazy-total, or worker ceiling change. Cross-domain texture/sampler and incompatible-geometry retirement add about 0.2 kB gzip to the synchronous/deployed path, preserve the lazy and worker totals, and raise only the four affected rounded ceilings by 150–250 bytes. |
| Safari 17/A10 and Quest 2 physical behavior | partial | Browser/Quest harnesses and telemetry exist. Examples embed a full revision, dirty flag, build timestamp and unique build ID in a no-store server endpoint and reports. Smoke, route-benchmark and glTF-load harnesses require the served identity to match the current emitted build; managed runs also require their own preview readiness, so an occupied port cannot silently redirect evidence to an older process. HTTP/body, managed-preview and CDP-socket readiness each have explicit deadlines; ANSI-colored process output is normalized before matching, so a live but unusable endpoint or presentation-only banner cannot hang CI or device work. Headless CI tracks stable Chrome-for-Testing and explicitly permits SwiftShader as a behavior oracle only; hardware and performance claims require physical evidence. The iPad harness rejects stale/mismatched builds and terminates boundedly after inspector failure. Exact clean Safari build `77a79eab` settles Sponza's 69/69 images with zero fallback, failure or GPU denial, then completes 60/60 full-DPR motion samples at 15.4 ms average and 19 ms p95; its close-ground run retains 10 pages with no pending/failure and completes 24/24 samples at 18 ms p95. Exact read-ahead build `9f697918` repeats Sponza under a cold cache and concurrent host load: first usable at 6.656 seconds, 69/69 images at 8.656 seconds, no fallback/failure/denial/interruption, and 24/24 full-DPR motion frames at 27 ms p95. This noisier run is correctness/load evidence rather than a replacement steady-state floor. Earlier selector-addressed Bistro Exterior/Interior/Interior Wine evidence settles every 202/111/110 image without fallback, failure, denial or application browser diagnostic and completes 30/30 moving frames at 34/28/24 ms p95. Exact clean Quest build `278cbdc7` closes the immersive device row: the Adreno 650 enters one 90 Hz session with two 2880x1584 views, completes 180/180 samples at 11.27 ms average / 14.34 ms p95 and 1.00 ms Royal-callback p95, settles the Tiger SVG without fallback/failure, retains a coherent stereo framebuffer, and explicitly exits. Exact clean build `77a79eab` revalidates its non-immersive close-ground path at 10 cm with 5-to-11 resident-page refinement, no pending/failure or browser diagnostic, and thermal status `none`; physical XR activation was not repeated because the device proximity sensor reported unworn. The accepted immersive run meets the 60 Hz floor, while its 14.34 ms p95 does not establish sustained 90 or 120 Hz. Sponza's iPad p95 remains above 16.67 ms, while Bistro and A Beautiful Game remain below the desired device floor; those are measured performance constraints rather than missing evidence. |
| Probability native-resolution PBR floor | partial | On physical Safari 17.14/A10 at a 2048-by-1008 backing size, settled Settlers camera motion measures 27--32 ms median / 37--40 ms p95 at about 82 submissions and 44 texture binds per frame; a 52-card SVG import measures roughly 47--49 ms median / 55--60 ms p95 at about 61 submissions. Probability input work remains near 1 ms. A 2026-08-10 matched local-build A/B found one smaller owner: forcing the exact position-only depth pass for this outside-camera workload improved two 5-second controls at 36 ms median / 43--44 ms p95 to 32--33 / 39--40 ms. Unconditional admission was rejected because 120-sample Bistro Exterior regressed from 33 / 36 ms median/p95 and 147 submissions to 34 / 38 ms and 192 submissions. Royal instead retains a cold, allocation-free outside-view classifier requiring a direct multisampled target and at least 2× camera-facing retained-bound coverage. A corrected wall-clock-path Settlers A/B measured 33 / 39 ms against 37 / 44 ms control, and the cleaned candidate repeated at 30 / 38 ms. Bistro's single-sample composite excludes the branch and retains exactly 147 submissions. Once admitted, invariant covered color draws now require exact depth equality; alternating physical candidates measured 27 / 34 and 26 / 34 ms median/p95 versus less-or-equal controls at 28 / 55 and 31 / 38 ms. Mixed standard, unlit and blended output plus the device's missing `EXT_float_blend` prevents the current retained HDR composite from becoming a quality-equivalent single-sample experiment; asynchronous occlusion results cannot conservatively control moving same-frame visibility. Suppressing redundant color-pass depth writes after the exact prepass measured the same 32 / 39 ms median/p95 as its physical control and was reverted. Wrong-material collapse, global opaque sorting, constant-normal specialization, hoisted GGX work, disabled alpha compositing and disabled MSAA were likewise insufficient, neutral or regressions and are not retained. The remaining constraint is native fragment/material cost plus a smaller hidden-fragment and submission component. Royal owns any future exact optimization; resolution reduction, weakened material semantics, device tiers and Probability hints are rejected for the golden path. |

The 2026-08-10 Probability continuation retains two further exact depth-state
results. Invariant prepass color draws use `EQUAL`; alternating physical
Settlers candidates measured 27 / 34 and 26 / 34 ms median/p95 versus
less-or-equal controls at 28 / 55 and 31 / 38 ms. Royal also discards its
default framebuffer depth only after all ordinary canvas passes. Settlers then
measured 20 / 25 and 19 / 24 ms around a 26 / 32 ms control. Bistro retained
147 submissions, all 202 images, and an RMSE 0 capture at 28 / 35 ms. The
analogous offscreen-composite discard regressed Bistro to 33 / 36 ms versus its
immediately reverted 24 / 29 ms control and is not retained. External and XR
framebuffers remain caller-owned. A following full-DPR Sponza run completed
120/120 moving frames at 22 / 26 ms median/p95 with 69/69 images, no warning or
failure, and a clean capture; it is correctness evidence rather than a new
performance floor.

The invariant equality state adds 10--75 gzip bytes across affected synchronous
graphs. Final default-depth discard adds a further 14--29 bytes; its five narrow
ceilings move one 50-byte step, while lazy and worker budgets do not increase.

Against clean baseline `bc0e41eb`, explicit non-visual glTF claims add 581
initial and 586 total deployed gzip bytes. Worker payload is unchanged and the
five-byte lazy-total movement is bundler chunk variance, not a new lazy feature.
The cost owns the validated identity constructor, React/root contract, atomic
visual/non-visual handoff, validation, and one shared preparation lifecycle; it
adds no frame, GL, picking, or retained geometry path. Selected material images
use the existing bounded texture lifecycle. Only the
five affected rounded initial/total ceilings rise, by 250--500 bytes according
to their prior slack. Declarations, source maps, and implementation raise the
packed renderer from 525,806 to 527,570 bytes, so its coarse ceiling moves from
514 to 516 KiB.
...
extension-aware texture-source selection with canonical preparation. Direct
planner tests cover scene isolation, recursive material LOD, variants, and
AVIF/WebP/core choice; the complete suite covers SVG and material preparation
through the shared path. This removes the former all-images range overfetch
without a second texture interpretation path. Lazy, worker, and deployed-byte
ceilings do not rise. Three initial fixtures from the preceding shared-I/O packet exceeded
...
New behavior enters the existing validation, canonical preparation, retained
selection, governed resource, and explicit submission boundaries. A second
renderer or compatibility path requires concrete consumer evidence and an
explicit conformance entry. Historical implementation plans remain available
in version control rather than in the normative reading order.
