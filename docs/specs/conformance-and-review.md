...
| Per-frame upload admission | partial | Separate root-local byte governors defer ordinary-texture, canonical geometry/instance, and VT page/page-table transfers consistently across canvas and XR frames. Each domain allows one oversize first transaction to prevent starvation; texture-only commits cannot consume geometry allowance. Render targets are allocation rather than source transfer. Prefiltered-environment source upload still needs admission or an explicit bounded-domain justification. |
