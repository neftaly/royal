...
| Bounded emissive volumes | partial | The public standalone node validates copied density/noise/extinction inputs, rejects zero scale eagerly, and lowers boxes plus closed outward convex triangle hulls before GL allocation. Its lazy WebGL owner retains exact proxy values rather than collision-prone compact keys, brackets sampleable opaque depth without changing non-volume composite storage, handles external perspective views, and retries budget denial only after released capacity. Focused descriptor, hull, ownership, collision, budget, packed-consumer, and real-browser route checks cover the shipped path. The implementation is deliberately a twelve-step full-resolution emissive approximation: transparent surfaces are always later, transmission snapshots exclude volume emission, overlapping media use ordered alpha, and no physical Safari/Quest performance evidence exists yet. |
