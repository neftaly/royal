...
| Safari 17/A10 and Quest 2 physical behavior | partial | Browser/Quest harnesses and telemetry exist. Examples embed a full revision, dirty flag, build timestamp and unique build ID in a no-store server endpoint and reports. Smoke, route-benchmark and glTF-load harnesses require the served identity to match the current emitted build; managed runs also require their own preview readiness, so an occupied port cannot silently redirect evidence to an older process. The iPad harness rejects stale/mismatched builds and terminates boundedly after inspector failure. Current exact clean build `6e6047ef` has a complete physical-Safari texture-integrity pass. Sponza settles 69/69 images with zero fallback, failure, GPU denial or browser diagnostics, then completes 60/60 full-DPR motion samples at 15.95 ms average and 19 ms p95 while retaining 222,049,536 / 268,435,456 persistent GPU bytes. Close-ground VT retains 10 pages with zero pending/failure and completes 24/24 frames at 18 ms p95. Selector-addressed Bistro Exterior/Interior/Interior Wine settle every 202/111/110 image with zero fallback, failure, denial or browser diagnostic and complete 30/30 moving frames at 34/28/24 ms p95 (`2026-07-21T23-35-01-292Z`, `23-36-18-224Z`, and `23-37-11-679Z`). Exterior first becomes usable at 7.61 seconds and needs 41.14 seconds for all textures. These captures close the current-build iPad wrong-texture/flicker regression check but do not turn a completed-frame sample into a sustained-vsync claim: Sponza's p95 remains above 16.67 ms, while Bistro and A Beautiful Game remain below the desired floor. Prior device comparisons and rejected diagnostics remain useful historical evidence, but this exact-build pass is the release oracle. Quest still needs a fresh controlled run, and no static spec can prove sustained 60/90/120 Hz or all visual oracles. |
