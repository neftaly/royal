...
| Resource admission/accounting | conforms | Root governor, typed leases, capacity wake, quarantine debt, VT/texture/target/geometry tests cover physical domains. Geometry arenas are byte-chunked and claimed lazily with their first admitted surface rather than reserved for the complete scene. Audit subsystem projections for overlap whenever new diagnostics are added. |
