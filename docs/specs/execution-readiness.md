...
| Validation, commit and release proof | The complete local replacement suite, glTF manifest, types, lint, builds, package entrypoints, VT benchmark build and bundle ceilings are repeatable gates. Exact clean commit `6e6047ef` contains and publishes the exact-budget replacement, complete authored sampler claims, React DX declarations, bounded VT fragment-path reduction, and the corrected execution record. The same emitted build has retained physical-iPad captures for Sponza, close-ground VT, and all three Bistro scenes. | Source publication and current-build iPad proof are complete. Exact-build Quest captures must still close the physical rows before release completion. |
