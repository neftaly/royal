...
| --- | --- | --- |
| Baseline PBR and static glTF fidelity | Core metallic-roughness, lights, IBL, normal/emissive/occlusion/specular/transmission/volume lowering, official asset manifest, pixel/decode gates, and required-extension failures pass the replacement suite. Exact-build physical iPad scene captures and a stereo Quest SVG/direct-surface capture retain the final pixels. | Complete for the accepted static profile. |
...
| VT2, SVG and ETC2 KTX2 | Authored/automatic VT share demand/publication/resource paths; near-plane and close-ground properties, GS SVG preferred/fallback behavior, and direct offline ETC2 upload/selection have focused and browser evidence. Exact clean build `6e6047ef` retains 10 close-ground VT pages on physical Safari with no pending or failed work. Exact clean build `278cbdc7` reaches distance 0.1 on Quest, grows from 5 to 11 resident pages with no pending/failure, and retains both canvas and physical-headset captures. | Complete for the accepted profile; Basis transcoding remains an explicit non-claim. |
...
| Performance, GC and deployed bytes | Retained packets/state, allocation-focused cores, exact resource accounting, upload governors, CPU/GPU traces and bundle attribution exist. Current local gates report 114.8 kB initial and 231.3 kB total deployed JavaScript gzip; Royal itself accounts for 55.5 kB initial and 172.0 kB across the complete optional graph. The selected-scene range slice adds about 0.1 kB initial and 2.2 kB deployed while removing 21.96 MB from the measured Bistro wine-scene geometry transfer. Exact Quest XR meets the accepted 60 Hz floor; its configured 90 Hz session does not meet a sustained 11.11 ms p95 claim. | CPU/GC is not the present heavy-scene limit. Sponza misses a sustained physical-iPad p95 claim; Bistro and A Beautiful Game remain texture/fragment-bound below the desired device floor. Offline GPU-compressed content is the strongest accepted route, not a renderer quality shortcut. |
| Validation, commit and release proof | The complete local replacement suite, glTF manifest, types, lint, builds, package entrypoints, VT benchmark build, bundle ceilings and all-route rendered-pixel smoke are repeatable gates. Exact clean build `278cbdc7` contains the final transparent-example and reliable Quest-harness slices. Physical evidence is retained for current-build iPad Sponza/VT/all Bistro scenes and Quest XR/close-ground VT, with matching source identities and device telemetry. | Complete for the accepted roadmap. Remaining performance limitations are measured product constraints, not missing proof. |
