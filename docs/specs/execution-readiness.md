...
| Performance, GC and deployed bytes | Retained packets/state, allocation-focused cores, exact resource accounting, upload governors, CPU/GPU traces and bundle attribution exist. Current local gates report 115.0 kB initial and 229.6 kB total deployed JavaScript gzip. Exact Quest XR meets the accepted 60 Hz floor; its configured 90 Hz session does not meet a sustained 11.11 ms p95 claim. | CPU/GC is not the present heavy-scene limit. Sponza misses a sustained physical-iPad p95 claim; Bistro and A Beautiful Game remain texture/fragment-bound below the desired device floor. Offline GPU-compressed content is the strongest accepted route, not a renderer quality shortcut. |
