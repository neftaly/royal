// Snapshot of the baseline probe with only the global UBO transport extended across blocks.
...
const fragment = (mode, capacity, brdf, blocks) => `#version 300 es
...
${mode === 'ubo' ? Array.from({length:blocks},(_,i)=>`layout(std140) uniform Lights${i} { vec4 records${i}[${capacity * 4}]; };`).join('\n') : mode === 'uniform' ? `uniform vec4 records[${capacity * 4}];` : 'uniform sampler2D lightData;'}
${mode === 'tiled' ? 'uniform usampler2D lightList;' : ''}
vec4 record(int index, int field) {
  ${mode === 'ubo' ? Array.from({length:blocks},(_,i)=>`${i<blocks-1?`if(index<${(i+1)*capacity}) `:''}return records${i}[(index-${i*capacity})*4+field];`).join('\n') : mode === 'uniform' ? 'return records[index*4+field];' : 'return texelFetch(lightData,ivec2(field,index),0);'}
}
${brdf}
...
export const run = async ({size=1024,samples=9,batch=4,orderSeed=0,blockRecords=Infinity,counts=[256,512],kinds=['directional','sparse','dense']}={}) => {
  ...
  const programFor=(mode,capacity,blocks)=>{
    const key=mode+':'+capacity+':'+blocks;
    if(programs.has(key))return programs.get(key);
    const started=performance.now(),vs=compile(gl.VERTEX_SHADER,vertex),fs=compile(gl.FRAGMENT_SHADER,fragment(mode,capacity,brdf,blocks));
  ...
  try{
    ...
    for(const count of counts)for(const kind of kinds){
      ...
      const modes=['texture','ubo'];
      for(const mode of modes){
        const capacity=mode==='ubo'?Math.min(Math.max(256,count),Math.floor(capabilities.maxUniformBlockSize/64),blockRecords):Math.max(1,count);
        const blocks=mode==='ubo'?Math.ceil(Math.max(1,count)/capacity):1;
        if(mode==='ubo' && blocks>capabilities.maxFragmentUniformBlocks){results.push({count,kind,mode,unsupported:'MAX_FRAGMENT_UNIFORM_BLOCKS'});continue;}
        if(mode==='uniform' && count>8)continue;
        const {program,compileAndLinkMs}=programFor(mode,mode==='texture'||mode==='tiled'?1:capacity,blocks);
        ...
        const buffers=[];let lightTexture=null,listTexture=null;
        const uploadStart=performance.now();
        if(mode==='ubo'){
          for(let block=0;block<blocks;block++){
            const storage=new Float32Array(capacity*16);storage.set(data.subarray(block*capacity*16,(block+1)*capacity*16));
            const buffer=gl.createBuffer();buffers.push(buffer);gl.bindBuffer(gl.UNIFORM_BUFFER,buffer);gl.bufferData(gl.UNIFORM_BUFFER,storage,gl.DYNAMIC_DRAW);gl.bindBufferBase(gl.UNIFORM_BUFFER,block,buffer);gl.uniformBlockBinding(program,gl.getUniformBlockIndex(program,'Lights'+block),block);
          }
        ...
        try{
          ...
          const entry={count,kind,mode,capacity,blocks,compileAndLinkMs,uploadSubmissionMs,lightBytes:mode==='ubo'?capacity*blocks*64:data.byteLength,listBytes:mode==='tiled'?list.indices.byteLength:0,listMs:mode==='tiled'?listMs:0,meanListCount:mode==='tiled'?list.meanCount:count,maxListCount:mode==='tiled'?list.maxCount:count,parity,gpuMs:stats(gpuMs),submissionMs:stats(submissionMs),completionMs:stats(completionMs)};
          results.push(entry);document.querySelector('#status').textContent=`${kind} ${count} ${mode}`;
        }finally{for(const buffer of buffers)gl.deleteBuffer(buffer);gl.deleteTexture(lightTexture);gl.deleteTexture(listTexture);}
      }
      await pause();
    }
    return {date:new Date().toISOString(),userAgent:navigator.userAgent,size,samples,batch,orderSeed,capabilities,source:'Royal 0.0.25 BRDF; research global texture versus multi-block UBO',results};
