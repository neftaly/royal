...
import { evaluate, spawnLogged, stopProcess } from '../../apps/examples-react/scripts/browser-harness.mjs';
...
try{
 ...
 await withResearchPage({debugHost:'127.0.0.1',debugPort:port,commandTimeoutMs:Number(process.env.LIGHTS_TIMEOUT_MS ?? 1800000)}, async session => {
 ...
 if(browser)await stopProcess(browser);if(profile)await rm(profile,{recursive:true,force:true,maxRetries:5,retryDelay:100});
}
