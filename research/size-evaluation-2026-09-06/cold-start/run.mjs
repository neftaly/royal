...
const output=process.env.ROYAL_COLD_OUTPUT??'/tmp/royal-cold-start/results.json';
...
try{
 for(let rep=0;rep<repetitions;rep++)for(const profile of ['normal','latency'])for(const scenario of ['card','dracoMain','dracoWorker']){
  for(const variant of rep%2?[candidate,'baseline']:['baseline',candidate]){
   ...
   const url=`http://127.0.0.1:3321/${profile}/${variant}/index.html?scenario=${scenario}`;
   for(const cache of ['cold','warm']){
    ...
    if(rep===0&&profile==='normal'&&cache==='cold')await page.screenshot({path:`/tmp/royal-cold-start/${variant}-${scenario}.png`});
   }
   await context.close();
  }
