...
export type StableRoyalLensStores = {
...
export const stableRoyalLensSchema = defineSchema({
...
const box = as(stableRoyalLensSchema.layoutBoxes, 'box');
const target = as(stableRoyalLensSchema.pickTargets, 'target');
const flag = as(stableRoyalLensSchema.renderFlags, 'flag');
const pointer = as(stableRoyalLensSchema.pointerSamples, 'pointer');
const effectResult = as(stableRoyalLensSchema.effectResults, 'effectResult');
const capabilityDiagnostic = as(stableRoyalLensSchema.capabilityDiagnostics, 'capabilityDiagnostic');
...
export const stableRoyalQueries = {
  renderRows: pipe(
    from(box),
    leftJoin(
      from(flag),
...
export function createStableRoyalStoreLenses(stores: StableRoyalLensStores): readonly StoreLens<unknown, Record<string, unknown>>[] {
  ...
  if (stores.documentStore !== undefined) {
    lenses.push(
      {
        relation: stableRoyalLensSchema.layoutNodes,
      ...
      {
        relation: stableRoyalLensSchema.assets,
        store: stores.documentStore,
...
export function createStableRoyalLensSnapshot(stores: StableRoyalLensStores): LensSnapshot {
  return createStoreLensSnapshot(createStableRoyalStoreLenses(stores), {
    diagnostics: stableRoyalProbeDiagnostics
  });
}

export function createStableRoyalAppBoundary(stores: StableRoyalLensStores): RoyalAppBoundary {
  return {
    snapshot: () => createStableRoyalLensSnapshot(stores),
    probe: () => createStableRoyalLensSnapshot(stores).probe,
    query: async (query) => evaluate(createStableRoyalLensSnapshot(stores).source, query)
  };
}

export async function evaluateStableRoyalLens<Row>(
  stores: StableRoyalLensStores,
  query: Query<Row>
): Promise<QueryResult<Row>> {
  return evaluate(createStableRoyalLensSnapshot(stores).source, query);
}
...
export function stableRoyalActivationPatchRoute(store: WritableStore<RoyalInteractionState>): AnyStorePatchRoute {
  return {
    relation: stableRoyalLensSchema.activationStates,
...
export function stableRoyalEffectResultPatchRoute(store: WritableStore<CapabilityRuntimeState>): AnyStorePatchRoute {
  return {
    relation: stableRoyalLensSchema.effectResults,
...
export function stableRoyalProbeDiagnostics(probe: LensProbe): readonly TarstateDiagnostic[] {
  const diagnostics: TarstateDiagnostic[] = [];
  const pickTargets = probe.rows(stableRoyalLensSchema.pickTargets);
  const targetIds = new Set(pickTargets.map((row) => scopedKey(row.scopeId, row.targetId)));
  const boxIds = new Set(probe.rows(stableRoyalLensSchema.layoutBoxes).map((row) => scopedKey(row.scopeId, row.boxId)));
  const assetIds = new Set(probe.rows(stableRoyalLensSchema.assets).map((row) => scopedKey(row.scopeId, row.assetId)));

  for (const pickTarget of pickTargets) {
    if (!boxIds.has(scopedKey(pickTarget.scopeId, pickTarget.boxId))) {
      diagnostics.push(missingRef('pick target points at missing layout box', stableRoyalLensSchema.pickTargets.name, 'boxId', pickTarget.targetId));
    }
  }

  for (const renderFlag of probe.rows(stableRoyalLensSchema.renderFlags)) {
    if (!boxIds.has(scopedKey(renderFlag.scopeId, renderFlag.boxId))) {
      diagnostics.push(missingRef('render flag points at missing layout box', stableRoyalLensSchema.renderFlags.name, 'boxId', renderFlag.boxId));
    }
  }

  for (const state of probe.rows(stableRoyalLensSchema.activationStates)) {
    for (const field of ['activeId', 'focusedId', 'hoveredId'] as const) {
      ...
      if (targetId !== undefined && !targetIds.has(scopedKey(state.scopeId, targetId))) {
        diagnostics.push(missingRef(`activation state points at missing ${field}`, stableRoyalLensSchema.activationStates.name, field, targetId));
      }
    }
  }

  for (const sample of probe.rows(stableRoyalLensSchema.pointerSamples)) {
    if (sample.targetId !== undefined && !targetIds.has(scopedKey(sample.scopeId, sample.targetId))) {
      diagnostics.push(missingRef('pointer sample points at missing pick target', stableRoyalLensSchema.pointerSamples.name, 'targetId', sample.sampleId));
    }
  }

  for (const assetFailure of probe.rows(stableRoyalLensSchema.assetDiagnostics)) {
    if (!assetIds.has(scopedKey(assetFailure.scopeId, assetFailure.assetId))) {
      diagnostics.push(missingRef('asset diagnostic points at missing asset row', stableRoyalLensSchema.assetDiagnostics.name, 'assetId', assetFailure.diagnosticId));
    }

    diagnostics.push({
      code: 'unreadable_ref',
      message: assetFailure.message,
      relation: stableRoyalLensSchema.assets.name,
      field: 'src',
...
function reduceRoyalActivationPatch(state: RoyalInteractionState, patch: WritePatch): RoyalInteractionState | undefined {
  if (patch.relation.name !== stableRoyalLensSchema.activationStates.name) {
    return undefined;
  }
...
function reduceRoyalEffectResultPatch(state: CapabilityRuntimeState, patch: WritePatch): CapabilityRuntimeState | undefined {
  if (patch.relation.name !== stableRoyalLensSchema.effectResults.name) {
    return undefined;
  }
