(() => {
  ...
  const copy = gl.copyTexSubImage2D;
  let copyCalls = 0, copiedPageEquivalents = 0;
  gl.copyTexSubImage2D = function (...args) {
    copyCalls++; copiedPageEquivalents += args[6] * args[7] / (132 * 132);
  ...
  const zoom = async (name, deltaY) => {
    const callsBefore = copyCalls, pagesBefore = copiedPageEquivalents;
    ...
    for (;;) {
      ...
      const vt = snapshot();
      const ms = now - start;
      if (vt.desiredPages === vt.admittedPages && vt.visibleDetailFraction >= .5) run.detail50Ms ??= ms;
      if (vt.desiredPages === vt.admittedPages && vt.visibleDetailFraction >= .9) run.detail90Ms ??= ms;
      ...
      if (vt.desiredPages === vt.admittedPages && vt.pendingPages === 0 && vt.unresidentPages === 0) {
        run.copyCalls = copyCalls - callsBefore; run.copiedPages = copiedPageEquivalents - pagesBefore;
  ...
  void (async () => {
    ...
    await sleep(1000);
    await zoom('zoom in after 1s idle', -500);
  })().catch(error => {report.error=String(error);}).finally(() => {
    gl.copyTexSubImage2D = copy;
