...
const sources = {
  before: await readFile('/tmp/royal-vt-before-pool-finalization.ts', 'utf8'),
  after: await readFile('/tmp/royal-vt-pool-finalization-candidate.ts', 'utf8'),
};
const functions = Object.fromEntries(Object.entries(sources).map(([version, source]) => {
  ...
  const body = source.slice(start, end).replaceAll('this.#', 'state.');
...
const stateFor = resources => ({ resources: new Map(resources.map((r, i) => [i, r])),
  atlasDemand: new Map(), atlasDemandBytes: new Map(), atlasMinimumSlots: new Map(),
...
const results = [];
for (const count of [1, 16, 128]) for (const pools of [1, Math.min(count, 8)]) {
  const resources = Array.from({ length: count }, (_, i) => ({ pool: String(i % pools), desiredPageCount: i % 3 ? 21 : 0,
    manifest: { pageEncoding: 'image', pageBytes: (i % pools + 1) * 69696 } }));
  for (const fn of Object.values(functions)) { const state = stateFor(resources); for (let i = 0; i < 10000; i++) run(fn, state); }
  for (let round = 0; round < 4; round++) for (const version of round % 2 ? ['after', 'before'] : ['before', 'after']) {
    ...
    results.push({ version, count, pools, round, elapsedMs, cpuMs: (usage.user + usage.system) / 1000, checksum });
  }
}
await writeFile('research/vt-comparison/pool-finalization-comparison.json', JSON.stringify({
